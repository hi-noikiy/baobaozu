...
<script id='goods_info'  type='text/html'>
    ...
    <div class="good_info1">
        ...
        <div class="price">
			...
			<!---lank-添加租金-->
			<d id='productsn'>
				<%if goods.minproductsn !=goods.maxproductsn%>租金:
                	<%goods.minproductsn%> - <%goods.maxproductsn%>
                <%else if goods.productsn!='' %>
                	<%goods.productsn%>
                <%/if%>
			</d>&nbsp;&nbsp;
			<d id='marketprice' style="font-size:12px; color:#333">押金:
         ...
         setoptions();
			...
			if(ret.no==''){
			   ...
			   $("#optionid").val(optionid); 
				...
				$("#marketprice").html("押金:"+marketprice);
				//-lank- 添加租金
				if(productsn==""){
				$('#productsn').html("");
				}
				else{
				   $('#productsn').html("租金:"+productsn);
				}
