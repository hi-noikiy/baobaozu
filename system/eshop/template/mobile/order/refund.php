 ...
 <div class="refund_main" >
        ...
        <div class="line">
            ...
            <div class="info">
                <div class="inner">
                <select id="reason">
					 <option value="合同到期" <%if refund.reason=='合同到期'%>selected<%/if%>>合同到期</option><!---lank--->
        ...
        <div class="line"><div class="label">退款说明</div><div class="info"><div class="inner"><input type="text" id="content" placeholder="退货快递单号(顺丰:67******)" value="<%if refund.content%><%refund.content%><%/if%>"/></div></div></div>
           ...
           core_json("<?php echo 	create_url('mobile',array('do'=>'order','act'=>'op','m'=>'eshop'))?>",{op:'refund',orderid:orderid},function(json){
                   ...
                   function bindEdit(){
                            ...
                            $('#refund_submit').click(function(){
							   //--lank--	
							   var daqi =$('#reason').val();
							   var daiqi_shuoming = $('#content').val();
							   //alert(daqi+'--'+daiqi_shuoming) 
							   if(daqi == '合同到期' && daiqi_shuoming=='' ){
								  alert('选择合同到期，退款说明必须填写退货快递单号！')
								  return false; 
							   }
							   //--lank-end- 	
                               if($(this).attr('saving')=='1'){
