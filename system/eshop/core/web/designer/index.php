...
if ($op == 'api') {
	if ($apido == 'setdefault') {
        ...
        if ($apido == 'selectgood') {
            $kw    = $_GPC['keyword'];
            $list = pdo_fetchall("SELECT id,title,productprice,marketprice,productsn,thumb,sales,unit FROM " . tablename('eshop_goods') . " WHERE uniacid= :uniacid and status=:status and deleted=0 AND title LIKE :title ", array(
                ':title' => "%{$kw}%",
                ':uniacid' => $_W['uniacid'],
                ':status' => '1'
            ));/*-lank- 添加productsn*/
