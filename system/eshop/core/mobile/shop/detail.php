...
if ($_W['isajax']) {
	...
	$productsn    = $goods['productsn'];/*-lank- 添加租金*/
	$maxproductsn = $productsn;/*-lank- 添加租金*/
    $minproductsn = $productsn;/*-lank- 添加租金*/
    ...
    $options = array();
    if (!empty($goods['hasoption'])) {
        $options = pdo_fetchall("select id,title,thumb,marketprice,productprice,productsn,costprice, stock,weight,specs from " . tablename('eshop_goods_option') . " where goodsid=:id order by id asc", array(
            ':id' => $goodsid
        ));
        ...
        foreach ($options as $o) {
			...
			if ($maxproductsn < $o['productsn']) {
                $maxproductsn = $o['productsn'];
            }/*-lank- 添加租金*/
            if ($minproductsn > $o['productsn'] && $o['productsn'] > 0) {
                $minproductsn = $o['productsn'];
            }/*-lank- 添加租金*/
        }
        $goods['maxprice'] = $maxprice;
        $goods['minprice'] = $minprice;
		$goods['maxproductsn'] = $maxproductsn;/*-lank- 添加租金*/
        $goods['minproductsn'] = $minproductsn;/*-lank- 添加租金*/
    }
    $specs  = $allspecs;
