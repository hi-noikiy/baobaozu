...
function goods_getList($args = array())
	{
		...
		$total = '';
			/*-lank- 添加productsn 显示租金*/
		if (!$random) {
			$sql = 'SELECT id,title,thumb,marketprice,productprice,productsn,sales,total,description FROM ' . tablename('eshop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
			$total = pdo_fetchcolumn('select count(*) from ' . tablename('eshop_goods') . ' where 1 ' . $condition . ' ', $params);
		}
		else {
			$sql = 'SELECT id,title,thumb,marketprice,productprice,productsn,sales,total,description FROM ' . tablename('eshop_goods') . ' where 1 ' . $condition . ' ORDER BY rand() LIMIT ' . $pagesize;
			$total = $pagesize;
		}
